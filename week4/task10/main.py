...
def main():
    ...
    try:
        ...
        pose_multi_joints = np.array([0.0, -90.0, 0.0, -15.0, 0.0, -40.0], dtype=np.float64)
        print("\n>> 动作4:关节 1~6 协同多轴联动 ->", pose_multi_joints[:5])
        current_pose = move_joint_interpolated(ser, current_pose, pose_multi_joints, duration=2.0)
        time.sleep(2.0)

        pose_multi_joints = np.array([0.0, -90.0, 0.0, -15.0, 0.0, 0.0], dtype=np.float64)
        print("\n>> 动作5:关节 1~6 协同多轴联动 ->", pose_multi_joints[:5])
        current_pose = move_joint_interpolated(ser, current_pose, pose_multi_joints, duration=2.0)
        time.sleep(2.0)
        #动作3
        pose_multi_joints = np.array([0.0, -10.0, -60.0, -15.0, 0.0, -40.0], dtype=np.float64)
        print("\n>> 动作6:关节 1~6 协同多轴联动 ->", pose_multi_joints[:5])
        current_pose = move_joint_interpolated(ser, current_pose, pose_multi_joints, duration=2.0)
        time.sleep(2.0)
        ...
        # pose_multi_joints = np.array([0.0, -90.0, 0.0, -5.0, 0.0, -40.0], dtype=np.float64)
        # print("\n>> 动作4:关节 1~6 协同多轴联动 ->", pose_multi_joints[:5])
        # current_pose = move_joint_interpolated(ser, current_pose, pose_multi_joints, duration=2.0)
        # time.sleep(1.0)

        # pose_multi_joints = np.array([0.0, -90.0, 0.0, -5.0, 0.0, 0.0], dtype=np.float64)
        # print("\n>> 动作5:关节 1~6 协同多轴联动 ->", pose_multi_joints[:5])
        # current_pose = move_joint_interpolated(ser, current_pose, pose_multi_joints, duration=2.0)
        # time.sleep(1.0)
        #  #动作1 
